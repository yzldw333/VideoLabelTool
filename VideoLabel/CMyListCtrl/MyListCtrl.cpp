...
CMyListCtrl::CMyListCtrl()
{
	...
	m_SSelectItemTextColor = GetSysColor(COLOR_BTNTEXT);//选中行文本颜色
	m_SSelectItemBkColor = GetSysColor(COLOR_HIGHLIGHT);//选中行背景颜色
	m_nHoverIndex=-1;
	m_bTracking=FALSE;
}
...
void CMyListCtrl::SetSSelectItemBkColor(COLORREF color, BOOL bDraw)//设置选中行背景颜色
{
...
void CMyListCtrl::SetSSelectItemTextColor(COLORREF color, BOOL bDraw)//设置选中行文本颜色
{
	...
	*pResult = 0;
	...
	if (pNMCD->nmcd.dwDrawStage==CDDS_PREPAINT){
		*pResult = CDRF_NOTIFYITEMDRAW;
		...
		}else if(nItemIndex %2 ==1){	//奇数行 比如 1、3、5、7
